...
const Page = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('token', JSON.stringify(res?.data?.attributes?.tokens?.access?.token));

      toast.success('Login successful', { theme: "colored" });
      window.location.href = '/';
      
    } catch (error) {
      toast.error(error?.data?.message || "Login failed", { theme: "colored" });
    }
  };
  ...
  return (
    <div className="bg-[url('/Images/Home/preduction-2.png')] w-full min-h-screen bg-cover bg-center flex items-center justify-center">
      ...
      <form onSubmit={handleLogin} className='w-full max-w-md border-2 border-purple-700 p-6 rounded-lg bg-white shadow-md'>
        ...
        <p className='mb-5 text-sm text-gray-600'>Please enter your email and password to continue</p>

        {/* Email */}
        <div className='mb-4'>
          <label htmlFor='email' className='font-semibold block mb-1'>Email</label>
          <input type='email' id='email' name='email' placeholder='Enter your email'
            required className='w-full p-2 border border-purple-700 rounded-md focus:outline-none' />
        </div>
